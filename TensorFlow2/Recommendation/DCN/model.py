...
class Dlrm(tf.keras.Model):
    ...
    def create_cross_layer(self):
        with tf.name_scope('data_parallel'):
            with tf.name_scope('top_mlp'):
                self.cross_weight = [
                    self.add_weight(name='cross_layer_w_'+str(i),
                                    shape=(self.cross_input_shape, 1),
                                    initializer='random_normal',
                                    regularizer=l2(0),
                                    trainable=True)
                    for i in range(self.num_of_cross_layer)
                                    ]
                self.cross_bias = [
                    self.add_weight(name='cross_layer_b_'+str(i),
                                    shape=(self.cross_input_shape, 1),
                                    initializer='random_normal',
                                    regularizer=l2(0),
                                    trainable=True
    ...
    def call(self, inputs, sigmoid=False, training=False):
        ...
        # print('*'*20, x.shape)
        cross_output = self._call_cross_layer(x)
        deep_output = self._call_top_mlp(x, training=training)

        total_output = tf.concat([cross_output, deep_output], axis=-1)

        # print('&'*20, cross_output.shape, deep_output.shape, total_output)

        x = self.final_out(total_output)
        if sigmoid:
            x = tf.math.sigmoid(x)
    ...
    def _call_cross_layer(self, inputs):
