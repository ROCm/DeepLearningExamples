...
class Dlrm(tf.keras.Model):
    def __init__(self, FLAGS, dataset_metadata, multi_gpu_metadata):
        ...
        # fwfm add
        self.num_of_cross_layer = FLAGS.num_cross_network_layers
        self.cross_input_shape = (len(FLAGS.synthetic_dataset_cardinalities) + 1) * (FLAGS.embedding_dim)
        self.create_fwfm_layer()
    ...
    def call(self, inputs, sigmoid=False, training=False):
        ...
        # Linear Part
        x = tf.concat([interaction_input[:, i, :] for i in range(interaction_input.shape[1])], axis=-1)
        linear_part = self.linear_part_layer(x)

        # 2-order part
        pairwise_inner_prods = []
        for fi in range(self.num_all_categorical_features):
            for fj in range(fi+1, self.num_all_categorical_features):
                r_ij = self.field_strengths[fi, fj]

                feat_embed_i = tf.squeeze(interaction_input[0:, fi: fi+1, 0:], axis=1)
                feat_embed_j = tf.squeeze(interaction_input[0:, fj: fj+1, 0:], axis=1)

                fwfm = tf.scalar_mul(r_ij, feat_embed_i * feat_embed_j)
                pairwise_inner_prods.append(fwfm)
        pairwise_inner_prods = tf.stack(pairwise_inner_prods, axis=1)
        pairwise_inner_prods = tf.reduce_sum(pairwise_inner_prods, axis=1)
        pairwise_inner_prods = self.final_out(pairwise_inner_prods)

        y = pairwise_inner_prods + linear_part
    ...
    def create_fwfm_layer(self):
        with tf.name_scope('data_parallel'):
            with tf.name_scope('top_mlp'):
                self.final_out = Dense(1, activation=None, use_bias=False)
                self.linear_part_layer = Dense(1, activation=None, use_bias=True) # fi
                self.field_strengths = self.add_weight(name='field_strenghts',
                                    shape=(self.num_all_categorical_features, self.num_all_categorical_features),
